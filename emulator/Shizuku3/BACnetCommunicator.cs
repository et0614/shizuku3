...
{
  /// <summary>
  /// NuGet 版 DeviceStorage には AddObject ヘルパが無いので、PoEMServer 側で
  /// 拡張メソッドとして提供する (動的に BACnet オブジェクトを追加するのに使用)。
  /// </summary>
  public static class DeviceStorageExtensions
  {
    public static void AddObject(this DeviceStorage storage, System.IO.BACnet.Storage.Object obj)
    {
      var arr = storage.Objects;
    ...
    /// <summary>
    /// 指定インスタンス番号範囲 [first, endExclusive) のオブジェクトを一括削除する
    /// (Device オブジェクトは対象外)。子機のアドレス変更時にブロック単位で撤去するのに使う。
    /// </summary>
    public static void RemoveObjectsInRange(this DeviceStorage storage, uint first, uint endExclusive)
    {
      storage.Objects = storage.Objects
        .Where(o => o.Type == System.IO.BACnet.BacnetObjectTypes.OBJECT_DEVICE
                 || o.Instance < first || o.Instance >= endExclusive)
        .ToArray();
    }
  }
  ...
  public class BACnetCommunicator
  {
    ...
    public BACnetCommunicator(DeviceStorage storage, int exclusivePort, string localEndPointIP)
    {
      ...
      //BacnetIpUdpProtocolTransport bUDP;
      //標準ポートの場合には排他的に0xBAC0（47808）を使えると仮定
      //if (bacPort == 0xBAC0) bUDP = new BacnetIpUdpProtocolTransport(0xBAC0, true, false, 1472, localEndPointIP);
      //それ以外のポートの場合には排他的に使えないと仮定し、ブロードキャスト用の0xBAC0と他の排他的ポートを組み合わせる
      //else bUDP = new BacnetIpUdpProtocolTransport(0xBAC0, bacPort, false, 1472, localEndPointIP);
      //2025.11.28修正：常に排他的に使う設定とする
      //bUDP = new BacnetIpUdpProtocolTransport(0xBAC0, bacPort, false, 1472, localEndPointIP);
